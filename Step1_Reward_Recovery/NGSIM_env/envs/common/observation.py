...
class KinematicObservation(ObservationType):
    ...
    def space(self):
        return spaces.Box(shape=(24,), low=-1, high=1, dtype=np.float32)
    ...
    def observe(self):
        # Add ego-vehicle
        lcv_df = pandas.DataFrame.from_records([self.env.lcv_veh.to_dict()])
        lcv_df['ax'] = self.env.lcv_veh.acc_x
        lcv_df['ay'] = self.env.lcv_veh.acc_y
        lcv_df = lcv_df[self.features]

        fv_df = pandas.DataFrame.from_records([self.env.fv_veh.to_dict()])
        fv_df['ax'] = self.env.fv_veh.acc_x
        fv_df['ay'] = self.env.fv_veh.acc_y
        fv_df = fv_df[self.features]

        nlv_df = pandas.DataFrame.from_records([self.env.nlv_veh.to_dict()])
        nlv_df['ax'] = self.env.nlv_veh.acc_x
        nlv_df['ay'] = self.env.nlv_veh.acc_y
        nlv_df = nlv_df[self.features]

        olv_df = pandas.DataFrame.from_records([self.env.olv_veh.to_dict()])
        olv_df['ax'] = self.env.olv_veh.acc_x
        olv_df['ay'] = self.env.olv_veh.acc_y
        olv_df = olv_df[self.features]

        # Normalize
        lcv_df.columns = ['y', 'x', 'vx', 'vy', 'ax', 'ay']
        fv_df.columns = ['y', 'x', 'vx', 'vy', 'ax', 'ay']
        nlv_df.columns = ['y', 'x', 'vx', 'vy', 'ax', 'ay']
        olv_df.columns = ['y', 'x', 'vx', 'vy', 'ax', 'ay']
        lcv_df = self.normalize_obs(lcv_df).values
        fv_df = self.normalize_obs(fv_df).values
        nlv_df = self.normalize_obs(nlv_df).values
        olv_df = self.normalize_obs(olv_df).values

        # Flatten
        obs = np.concatenate([lcv_df, fv_df, nlv_df, olv_df]).reshape(-1)
