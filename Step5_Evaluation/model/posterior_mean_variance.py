...
class MeanProcessor(ABC):
    ...
    @abstractmethod
    def __init__(self, betas, dynamic_threshold=None, clip_denoised=None):
...
class EpsilonXMeanProcessor(MeanProcessor):
    def __init__(self, betas, dynamic_threshold, clip_denoised):
        ...
        alphas_cumprod_prev = torch.from_numpy(np.append(1.0, alphas_cumprod[:-1].numpy())).to(alphas.device)
...
class LearnedRangeVarianceProcessor(VarianceProcessor):
    def __init__(self, betas):
        ...
        self.posterior_log_variance_clipped = torch.from_numpy(np.log(
